...
foreach (var commandRow in read.Skip(rowNumber + 1))
{
    ...
    for (var i = 0; i < howManyToMove; i++)
    {
        var posFromValue = stackLists.Keys.First(x => x.Value == moveFromColumn).Pos;
        var toMove = stackLists[(posFromValue, moveFromColumn)].LastOrDefault();
        stackLists[(posFromValue, moveFromColumn)].RemoveAt(stackLists[(posFromValue, moveFromColumn)].Count() - 1);
        ...
        stackLists[(posToFromValue, moveToColumn)].Add((rowOnNew, toMove.Letter));
    }
}
